...
def save_log_level(self, level):
    if 'customData' in self.polyConfig:
        level_data = self.polyConfig['customData']
...
class NSParameters:
    ...
    def get_from_polyglot(self, poly):
        ...
        # check for any user added parameters
        for p in customParams:
            if not self.exists(p):
                self.internal.append({
                    'name': p,
                    'value': customParams[p], 
                    'default': '',
                    'isSet': True,
                    'isRequired': False,
                    'notice_msg': '',
                    })
                LOGGER.error('Adding User defined parameter ' + p)
